...
let exchange_fun = function(info) {
	...
	for (let i = 0; i < length; ++i) {
		...
		//console.log(info.data.records[i].consumType);
		if(info.data.records[i].customerName == null){
			info.data.records[i].customerName = "空（未登记）";
		}
	}
};
...
let staff_name_to_id = function(name) {
	...
	console.log("staff_name_to_id:"+ name +" " + id);
	return id;
};
...
let getDeleteId = function(obj) {
	deleteId = $(obj).parent().parent().children("#hide-id").text();
}
...
let change_consum_record_fun = function() {
	let changeConsumType = $('#change_consum_type').val(),
		...
		changeCustomerName = $('#change_customer_name').val();
		if(changeCustomerName == "空（未登记）"){
			changeCustomerName = null
		}
	$.ajax({
		...
		data: JSON.stringify({
			...
			remark: changeRemark,
			customerName: changeCustomerName
		}),
		success: function(info) {
			console.log(info);
